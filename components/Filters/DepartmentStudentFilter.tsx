...
export const DepartmentStudentFilter = ({
    onFilterChange,
    filter,
}: ItemFilterProps) => {
    ...
    const coordinatorOptions = coordinators?.data?.map((subadmin: any) => ({
        value: subadmin?.subadmin?.id,
        label: subadmin?.subadmin?.user?.name,
    }))
