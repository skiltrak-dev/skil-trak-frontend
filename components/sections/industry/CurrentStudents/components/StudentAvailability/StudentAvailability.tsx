...
export const StudentAvailability = ({ availability }: any) => {
   const [generalAvailability, setGeneralAvailability] = useState<any[] | null>(
      null
   )
   useEffect(() => {
      const weekdays = [
      ...
      const filteredDays = weekdays.filter((d) => !availabilityDays.includes(d))
         ...
         <div className="border border-dashed border-gray-400 rounded-lg p-1 flex flex-col justify-between gap-y-3">
            <div>
               <div className="grid grid-cols-5 gap-4 px-3">
                  {shifts.map((shift, i) => (
                     <Typography key={shift} variant={'xs'} capitalize center>
