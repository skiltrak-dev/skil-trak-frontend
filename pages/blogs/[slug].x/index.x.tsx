...
const Blogs: NextPageWithLayout = ({ data }: any) => {
    ...
    const categoryId = router.isReady ? router.query.category : null
    ...
    const filteredData =
        categoryId &&
        filterPublishedBlogs?.filter((item: any) =>
            item?.category?.some((cat: any) => cat.id === Number(categoryId))
        )
