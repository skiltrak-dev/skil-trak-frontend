      ...
      <div className="flex flex-col gap-y-12 items-start mt-8">
        ...
        {selected === UsageType[1].type && (
          <div className="flex flex-col">
            <TextInput onChange={(e: any) => { setStudentCapacity(e.target.value) }} name={'studentCapacity'} label={'Student Capacity'} type={'number'} />
          </div>
        )}
        <Button
          variant={'primary'}
          onClick={onSubmit}
          disabled={(selected === UsageType[1].type && !studentCapacity) || !selected}
          text={'Continue'}
        />
      </div>
