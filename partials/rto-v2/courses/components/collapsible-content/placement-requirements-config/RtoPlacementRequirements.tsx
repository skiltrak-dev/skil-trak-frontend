...
export const RtoPlacementRequirements = ({ course }: any) => {
    ...
    const [updateSummary, updateSummaryResult] = RtoV2Api.Courses.useUpdateCourseSummary();

    // Extract summary just once
    useEffect(() => {
        const summaryObj =
            course?.rtoCourseFiles?.find((f: any) => f.title === RtoFileTitle.LOGBOOK)
                ...
                <div className="bg-gradient-to-br from-accent/5 to-accent/10 rounded-xl border border-accent/20 overflow-hidden">
                    ...
                    <CollapsibleTrigger className="w-full p-4 flex items-start justify-between hover:bg-accent/5 transition-colors">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            ...
                            <Button Icon={isEditing ? Check : Edit}
                                onClick={(e) => {
                                    e.stopPropagation();
                                    isEditing ? handleSave() : setIsEditing(true);
                                }}
                    ...
                    <CollapsibleContent>
                        <div className="px-5 pb-5 space-y-4">
                            ...
                            {isEditing ? (
                                ...
                                <div
                                    className="prose prose-sm max-w-none bg-background/30 rounded-md p-4 border border-border/40 text-xs h-96 overflow-auto"
