...
import { ArrowRight, CheckCircle2, Eye, FileCheck, Info, Sparkles } from 'lucide-react';
import React from 'react'

export const LogbookSummaryDisplay = ({ course }: any) => {
    console.log('course', course);

    const rtoLogbookSummary = course.rtoCourseFiles.map((logbook: any) =>
        logbook?.rtoLogbookSummary?.length > 0 && logbook?.rtoLogbookSummary?.map(
            (summary: any) => summary
        )

    );

    console.log('rtoLogbookSummary:::', rtoLogbookSummary);
    ...
    return (
        <div>
            <div className="flex items-center gap-2 mb-4 px-4">
                ...
                <h3 className="font-semibold text-base">Auto-Extracted Logbook Summary</h3>
                <Badge
                    className={`ml-auto`}
                >
                    Uploaded
            ...
            <div className="rounded-xl bg-gradient-to-br from-accent/5 to-accent/10 border border-accent/30 p-5 space-y-4">
                ...
                <div className="grid grid-cols-4 gap-4 pb-4 border-b border-accent/20">
                    ...
                    <div>
                        <p className="text-xs text-muted-foreground mb-1">Placement Site</p>
                ...
                {/* Key Activities */}
                <div>
                    <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Key Activities</p>
                ...
                <div className="pt-2 border-t border-accent/20">
                    <p className="text-xs text-muted-foreground mb-1">Supervised By</p>
                ...
                <div className="flex items-start gap-2 p-3 rounded-lg bg-accent/10 border border-accent/20">
                    ...
                    <p className="text-xs text-muted-foreground">
                        <strong className="text-accent">Next Steps:</strong> Review the extracted summary, append it to RTO requirements if needed, then generate the assessment form with three sections for Student, Industry Supervisor, and RTO Assessor completion with e-sign capabilities.
                    </p>
                </div>
            </div>
        </div>
