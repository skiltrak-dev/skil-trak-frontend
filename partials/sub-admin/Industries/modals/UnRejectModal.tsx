    ...
    useEffect(() => {
        if (changeStatusResult.isSuccess) {
            notification.success({
                title: `Request Un Rejected`,
                description: `Industry "${industry?.user?.name}" has been Un Rejected.`,
            })
            onCancel()
        }
    }, [changeStatusResult])
    ...
    return (
        <>
            ...
            <ActionModal
                ...
                variant="primary"
                title="Are you sure!"
                description={`You are about to un-rejected "${industry?.user?.name}". Do you wish to continue?`}
