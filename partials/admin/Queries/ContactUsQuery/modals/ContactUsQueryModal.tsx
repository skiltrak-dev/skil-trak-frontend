...
export const ContactUsQueryModal = ({
    workBase,
    onCancel,
}: {
    workBase: any
        ...
        await contact(Number(workBase?.id)).then((res: any) => {
            if (res?.data) {
                notification.success({
                    title: `Contacted`,
                    description: `Contacted to ${workBase?.name}.`,
                })
                onCancel()
            }
        })
    ...
    return (
        <>
            ...
            <ActionModal
                ...
                variant="primary"
                title="Are you sure!"
                description={`You are about to contact "${workBase?.name}". Do you wish to continue?`}
                onConfirm={onConfirmUClicked}
                onCancel={onCancel}
                input
                inputKey={workBase?.name}
