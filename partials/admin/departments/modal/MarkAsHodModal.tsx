...
export const MarkAsHodModal = ({
    ...
    const [toggleHod, toggleHodResult] = AdminApi.Department.useToggleHod()
    ...
    useEffect(() => {
        if (toggleHodResult.isSuccess) {
            alert.success({
                ...
                description: `Coordinator "${item?.subadmin?.user?.name}" has been Marked as HOD.`,
            })
            onCancel()
        }
    ...
    return (
        <ActionModal
            ...
            variant="success"
            title="Are you sure!"
            description={`You are about to Mark as HOD <em>"${item?.subadmin?.user?.name}"<em>. Do you wish to continue?`}
