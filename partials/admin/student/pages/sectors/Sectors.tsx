    ...
    <Card>
      ...
      <div className="pl-1 ml-2 border-l border-secondary-dark">
        {sector?.courses?.map((sec: any) => (
          ...
          >
            <GoPrimitiveDot className="absolute top-auto bottom-auto -left-[15px] text-secondary-dark text-xl" />
            <div className="flex items-center gap-x-2">
              <GiOpenBook />
              <div>
                ...
                <Typography variant={'muted'}>{sec?.name}</Typography>
              </div>
            </div>
            <Typography variant={'muted'} color={'text-info'}>
              <span
                ...
                onClick={() => {
                  show()
                  setContent(<CourseDetails courseId={sec.id} />)
                }}
              >
                View
              </span>
