        ...
        return setIndustryChecks((preVal: any) => {
          return [
            ...preVal,
            {
              ...c,
              folders: c.folders
                .map((f: any) => f.documents.map((d: any) => d.response))
                .flat(Infinity),
            },
          ]
        })
  ...
  useEffect(() => {
    if (assessmentFolders) {
      ...
      assessmentFolders.map(({ course, ...folder }: any) => {
        ...
        if (newFolders.map((data) => data.title).includes(course?.title)) {
          newFolders[findIndex] = {
            ...newFolders[findIndex],
            folders: [...newFolders[findIndex].evidence, folder],
          }
        } else {
          newFolders = [
            ...newFolders,
        ...
        <div className="grid grid-cols-2">
          <div className="border-r border-secondary-dark pr-4 flex flex-col gap-y-4">
            ...
            {industryChecks
              ?.filter(
                (v, i, a) =>
                  industryChecks.findIndex((v2) => v2.id === v.id) === i
              )
              ?.map((course: any) => (
                <CourseFolders
                  key={course.id}
          ...
          <div className="pl-4 flex flex-col gap-y-4">
            ...
            {assessmentEvidenceFolders?.map((assessmentEvidenceFolder) => (
              <CourseFolders
