...
export const WPProcessingList = ({ workplaceId }: { workplaceId: number }) => {
    const [hoveredCriteria, setHoveredCriteria] = useState<number | null>(null)

    const router = useRouter()

    const getWorkplaceIndustryDetail =
        SubAdminApi.Student.getWorkplaceIndustryDetail(workplaceId, {
            skip: !workplaceId,
        })
                        ...
                        <div className="border-0 shadow-lg bg-white/95 backdrop-blur-md mt-3 relative overflow-hidden rounded-md p-0.5">
                            ...
                            <div className="p-4 relative z-10">
                                <div className="space-y-2 max-h-[50vh] overflow-auto custom-scrollbar">
                                    {getWorkplaceIndustryDetail?.isError && (
                                        <NoData
                                            isError
                                            text="There is some technical issue!"
                                        />
                                    )}
                                    {getWorkplaceIndustryDetail?.isLoading ? (
                                        <LoadingAnimation />
                                    ) : getWorkplaceIndustryDetail?.data
                                          ?.data &&
                                      getWorkplaceIndustryDetail?.data?.data
                                          ?.length > 0 &&
                                      getWorkplaceIndustryDetail?.isSuccess ? (
                                        getWorkplaceIndustryDetail?.data?.data?.map(
                                            (
                                                wpIndustries: any,
                                                index: number
                                            ) => {
