...
export const DistanceIndicator = ({ distance, mode = "walking" }: { distance: number; mode?: "walking" | "car" }) => {
    ...
    <div className="flex items-center gap-3 mt-2">
      ...
      <div className="relative w-24 h-6 overflow-hidden">
       ...
       <motion.div
            ...
            // animate={{ x: ["0%", "100%"] }}
            animate={{ x: [0, 90] }}
            transition={{ repeat: Infinity,  duration: mode === "car" ? 10 : 20, ease: "linear" }}
            >
            <MovingIcon size={16} />
        </motion.div>
      </div>
      ...
      <span className="text-xs text-gray-600">{Math.round(distance)} km away</span>
    </div>
  );
};
